...
if __name__ == "__main__":
    ...
    parser.add_argument("-R", "--raw", action='store_true', help="just pump out raw observation data")
    args = parser.parse_args()

    njob = noaa_job()
    if args.raw:
        items = njob.fordays(args.zipcode, args.country)
        print(f"{json.dumps(items, indent=3)}") 
    else:
