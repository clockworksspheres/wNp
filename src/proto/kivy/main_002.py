...
class Chart(Widget):
    ...
    def redraw(self, *args):
        ...
        with self.canvas:
            self.canvas.clear()  # Clear previous
        ...
        with self.canvas:
            ...
            for i, val in enumerate(self.values):
                ...
                bar_x = x - 12
                Line(
                    points=[bar_x, pad, bar_x, pad + bar_h, bar_x + 24, pad + bar_h, bar_x + 24, pad],
                    close=True
                )
        ...
        with self.canvas:
            ...
            for i, val in enumerate(self.values):
                ...
                line_pts.extend([x, y])
            Line(points=line_pts, width=2.5)
...
class WeatherApp(MDApp):
    ...
    def update_weather(self, *args):
        ...
        for key, val, chart_id, max_v in [
            ...
            chart.values = self.history[key][:]  # Triggers redraw
            chart.label = f"{key.capitalize()}: {val:.1f}"

        print(f"Graphing: T={temp:.1f}°C | H={humid:.0f}% | P={press:.0f}hPa")
