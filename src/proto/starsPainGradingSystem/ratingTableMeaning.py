...
class StarRatingDelegate(QStyledItemDelegate):
    ...
    def draw_star(self, painter, cx, cy, radius, filled):
        ...
        if filled:
            ...
            painter.setPen(QColor("#B8860B"))    # Dark gold outline
        else:
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.setPen(QColor("#666666"))    # Gray outline
    ...
    def editorEvent(self, event, model, option, index):
        # Clear hover when mouse leaves the cell
        ...
        # Only interested in mouse move and button press
        if event.type() not in (QEvent.Type.MouseMove, QEvent.Type.MouseButtonPress):
            return False

        # Ignore events outside the cell
...
class RatingTableModel(QAbstractTableModel):
    ...
    def get_all_data(self):
        """Return a list of dicts for saving"""
...
class TableApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PySide6 Interactive 5-Star Rating Table")
        ...
        legend = QLabel(
            ...
            "★★★★☆ Very Good<br>"
            "★★★☆☆ Average / Okay<br>"
            "★★☆☆☆ Fair / Disappointing<br>"
            "★☆☆☆☆ Poor / Terrible<br>"
            "(No stars = Unrated)"
        )
        legend.setAlignment(Qt.AlignmentFlag.AlignCenter)
        legend.setStyleSheet("QLabel { color: #444; font-size: 13px; margin: 10px; }")
    ...
    def save_ratings(self):
        ...
        try:
            ...
            QMessageBox.information(self, "Success", f"Ratings saved to:\n{file_name}")
