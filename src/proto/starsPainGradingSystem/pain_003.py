...
class PainStarDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # Determine color scheme based on the current displayed level
        if display_level == 0:
            fill_color = None  # No fill for zero
            outline_color = QColor("#666666")
        elif display_level <= 3:
            fill_color = QColor("#32CD32")      # Lime Green – Mild
            outline_color = QColor("#228B22")
        elif display_level <= 6:
            fill_color = QColor("#FFD700")      # Gold/Yellow – Moderate
            outline_color = QColor("#B8860B")
        else:
            fill_color = QColor("#FF4500")      # Orange-Red – Severe
    ...
    def draw_star(self, painter, cx, cy, radius, filled, fill_color, outline_color):
        outer = radius
        inner = radius * 0.382  # Golden ratio for nice star shape
    ...
    def sizeHint(self, option, index):
        return QSize(30 * 10 + 40, 55)  # Wide enough for 10 stars + padding
    ...
    def editorEvent(self, event, model, option, index):
        ...
        if event.type() == QEvent.Type.MouseButtonPress and event.button() == Qt.MouseButton.LeftButton:
            ...
            if new_level == current and current > 0:
                new_level = 0  # Re-click same star to clear
...
class PainTrackerApp(QWidget):
    def __init__(self):
        ...
        legend = QLabel(
            ...
            "<span style='color:#32CD32;'><b>●●●</b></span> 1–3 stars – <b>Mild pain</b> (noticeable but manageable)<br>"
            "<span style='color:#FFD700;'><b>●●●</b></span> 4–6 stars – <b>Moderate pain</b> (interferes with activities)<br>"
            "<span style='color:#FF4500;'><b>●●●</b></span> 7–10 stars – <b>Severe pain</b> (very difficult to function)<br><br>"
            "0 stars – No pain"
        )
    ...
    def save_ratings(self):
        file_name, _ = QFileDialog.getSaveFileName(
            self,
            "Save Pain Ratings",
            f"pain_ratings_{datetime.now().strftime('%Y-%m-%d')}.json",
